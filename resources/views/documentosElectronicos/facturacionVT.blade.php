...
@section('js')
    <script>
        ...
        function mostrarDatos(){
            DisposeGrids();
            TreeGrid({Layout:{Url:"gridLayoutFacturacionVT"},Data:{Url:"gridDataFacturacionVT/"+$("#dateInicio").val()+"/"+$("#dateFin").val()+"/"+$("#compania").val()},Debug:0},"facturacion");
        }
                ...
                if (R.length) {
                    for (var i = 0; i < R.length; i++) {
                        if (Is(R[i], "Selected")) {
                            ...
                            $.ajax({
                                type: "POST",
                                url: '{{URL::route("enviar.facturaVT")}}',
                                data: {INVOICE_ID: NO_FACTURA,
                                    COMPANY: $("#compania").val(),
                                    "_token": "{{ csrf_token() }}",},
                                success: function( msg ) {
